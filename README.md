...
- `api/routes`: HTTP surface for `auth`, `jobs` and `health`. Keep routes thin.
...
- `api/models`: ORM models for `User` and also `Job` on shared `Base`.
...
- Clear lane for AI logic via `api or services` so routes stay composable and testable.
...
Set `OPENAI_BASE_URL`, `OPENAI_API_KEY`, and `OPENAI_MODEL=DeepSeek-R1` in `ai_job_backend/.env` after deploying DeepSeek R1 in [Microsoft Foundry](https://ai.azure.com).
